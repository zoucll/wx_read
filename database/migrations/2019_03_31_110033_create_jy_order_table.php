...
class CreateJyOrderTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_order', function (Blueprint $table) {
            ...
            $table->string('order_sn',20)->default('')->comment('订单号');
            $table->integer('user_id')->default(0)->comment('用户id');
            $table->tinyInteger('order_status')->default(0)->comment('1 未确认  2已确认 3 取消 4 退货');
            $table->tinyInteger('shipping_status')->default(0)->comment('1 代发货 2 已发货 3 已收货 4 退货');
            ...
            $table->char('code',8)->default('')->comment('收货人的邮编');
            $table->char('phone',11)->default('')->comment('收货人手机号');
